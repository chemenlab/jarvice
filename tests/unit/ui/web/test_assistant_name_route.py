...
The handler is a plain ``def`` (threadpool route — it never awaits), so these
...
def test_warmup_without_config_answers_503_not_a_fake_name() -> None:
    with pytest.raises(HTTPException) as excinfo:
        get_assistant_name(_request_with_config(None))  # type: ignore[arg-type]
    assert excinfo.value.status_code == 503


def test_resolves_name_from_wake_phrase_when_config_present() -> None:
    ...
    payload = get_assistant_name(_request_with_config(cfg))  # type: ignore[arg-type]
