...
def test_search_happy_path_returns_scored_hits(
    ...
    # Search and writes must share the configured runtime DB. Point the legacy
    # fallback elsewhere so this catches accidentally opening the default DB.
    import sqlite3
    ...
    db.parent.mkdir()
    monkeypatch.setattr(
        "jarvis.memory.wiki.search._default_db_path", lambda: tmp_path / "wrong.db"
    )
