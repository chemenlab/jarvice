...
@pytest.mark.asyncio
async def test_correction_receives_full_existing_pages_with_russian_aliases(tmp_path: Path) -> None:
    """A Cyrillic name must reuse its Latin slug and retain dated prior facts."""
    from jarvis.memory.wiki.page import MarkdownPageRepository, parse_markdown

    pages = [
        parse_markdown(
            f"---\ntype: entity\nslug: {slug}\naliases: [{alias}]\n"
            f"created: 2026-09-11\n---\n# {alias}\n"
            f"Fictional QA character. {fact} Source: chat:2026-09-11.\n",
            tmp_path / "entities" / f"{slug}.md",
        )
        for slug, alias, fact in (
            ("aleksey-astronomer", "Алексей-астроном", "Violet tea; owns a telescope."),
            ("aleksey-baker", "Алексей-пекарь", "Mint tea; owns an oven."),
        )
    ]

    class Vault(FakeVault):
        def pages_by_type(self, page_type: str) -> list[Any]:
            return pages if page_type == "entity" else []

    brain = FakeBrain("[]")
    curator = WikiCuratorLLM(
        config=_make_config(), schema_path=_write_schema(tmp_path), registry=FakeRegistry(brain),
    )
    await curator.propose_updates(
        "Вымышленный Алексей-астроном теперь любит жасминовый чай; "
        "Алексей-пекарь по-прежнему любит мятный чай.",
        "chat:2026-09-13", repo=MarkdownPageRepository(), vault=Vault(),
    )
    prompt = brain.received_requests[0].messages[0].content
    for expected in (
        "entities/aleksey-astronomer.md", "entities/aleksey-baker.md",
        "owns a telescope", "owns an oven", "chat:2026-09-11", "2026-09-13",
    ):
        assert expected in prompt


def test_existing_page_context_never_truncates_a_page(tmp_path: Path) -> None:
    from jarvis.memory.wiki.page import MarkdownPageRepository, parse_markdown
    from jarvis.memory.wiki.prompt import select_existing_page_context

    page = parse_markdown(
        "---\ntype: entity\nslug: alex\n---\n# Алексей\n" + "x" * 500,
        tmp_path / "entities" / "alex.md",
    )
    context = select_existing_page_context(
        "Алексей", [page], MarkdownPageRepository(), max_chars=100,
    )
    assert context == [{"path": "entities/alex.md", "complete": False}]
