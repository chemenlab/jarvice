...
def test_repair_literal_dotted_table_preserves_legacy_fields_and_nested_precedence(tmp_path):
    f = tmp_path / "jarvis.toml"
    f.write_text('[trigger]\nwake_word_enabled = true\n["trigger.wake_word"]\nphrase = "Old"\nengine = "auto"\nfuzzy_match_ratio = 0.81\n[trigger.wake_word]\nengine = "vosk_kws"\n', encoding="utf-8")
    config_writer.set_wake_word("Привет, Джарвис", path=f)
    doc = _load(f)
    assert doc["trigger.wake_word"]["phrase"] == "Old"
    assert doc["trigger"]["wake_word"] == {
        "phrase": "Привет, Джарвис", "engine": "vosk_kws", "fuzzy_match_ratio": 0.81,
    }
