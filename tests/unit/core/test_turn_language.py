...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("Пожалуйста покажи мне какая погода будет завтра", "ru"),
        ("Please tell me who Александр Пушкин was", "en"),
        ("Spotify.", "unknown"),
        ("", "unknown"),
        ("GitHub", "unknown"),
    ],
...
def test_russian_pin_overrides_english_transcript() -> None:
    assert resolve_output_language(" RU ", "english", "Show me the calendar") == "ru"


def test_russian_conversation_keeps_thin_interjection() -> None:
    assert resolve_output_language(
        "auto", "english", "Okay", conversation_language="ru"
    ) == "ru"


def test_russian_output_is_validated_against_its_resolved_language() -> None:
    text = "Пожалуйста скажи мне какая погода будет завтра."
    assert validate_output_language(text, resolved_language="ru").status == "match"
    assert validate_output_language(text, resolved_language="en").should_block
