...
def test_russian_pin_uses_russian_model_and_preserves_native_data_root(tmp_path, monkeypatch):
    """A Russian install must not download English into the checkout's data dir."""
    monkeypatch.delenv("JARVIS__MEMORY__DATA_DIR", raising=False)
    monkeypatch.setenv("JARVIS_DATA_DIR", str(tmp_path))
    cfg = SimpleNamespace(trigger=_pin("ru-RU"), ui=SimpleNamespace(language="en"))
    assert wmf.resolve_wake_language(cfg) == "ru"
    blob = _fake_model_zip()
    monkeypatch.setitem(wmf.VOSK_MODELS, "ru", wmf.VoskModelSpec(
        "vosk-model-small-ru-0.22.zip", hashlib.sha256(blob).hexdigest(),
    ))
    def download(url):
        assert url.endswith("vosk-model-small-ru-0.22.zip")
        return blob
    installed = wmf.ensure_vosk_model("ru-RU", http_get=download)
    assert installed == tmp_path / "wake_models" / "vosk" / "ru"
    assert Path(resolve_vosk_model_path("ru")).is_relative_to(installed)
    assert wmf.vosk_model_present("ru")
