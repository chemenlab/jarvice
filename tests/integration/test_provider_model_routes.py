...
class _SubscriptionCatalog(_FakeCatalog):
    async def list_models(self, provider: str, *, force_refresh: bool = False) -> CatalogResult:
        self.calls.append((provider, force_refresh))
        return CatalogResult(provider, (
            ModelInfo("gpt-5.5", "GPT-5.5", input_modalities=("text", "image")),
            ModelInfo("new-native-model", "New native model", input_modalities=("text",)),
        ), "live", 123.0)


def test_subscription_models_refresh_keeps_verified_default(server: WebServer) -> None:
    server.app.state.model_catalog = _SubscriptionCatalog()
    with TestClient(server.app) as client:
        response = client.get("/api/providers/codex-subscription/models?refresh=true")
    assert response.status_code == 200
    body = response.json()
    assert body["current_model"] == "gpt-5.5"
    assert body["source"] == "live"
    assert {row["id"]: row["vision"] for row in body["models"]} == {"gpt-5.5": True, "new-native-model": False}
    assert server.app.state.model_catalog.calls == [("codex-subscription", True)]


def test_subscription_model_selection_requires_catalog_membership(server: WebServer, monkeypatch) -> None:
    server.app.state.model_catalog = _SubscriptionCatalog()
    server.app.state.brain = _FakeBrain(active="codex-subscription")
    monkeypatch.setattr("jarvis.ui.web.provider_routes._probe_brain_model", lambda *_: pytest.fail("Selecting a subscription model must not submit an unsolicited model turn"))
    with TestClient(server.app) as client:
        invalid = client.put("/api/providers/codex-subscription/model", json={"model": "invented-model", "persist": False})
        assert invalid.status_code == 422
        assert server.app.state.brain.applied == []
        valid = client.put("/api/providers/codex-subscription/model", json={"model": "new-native-model", "persist": False})
        assert valid.status_code == 200 and valid.json()["applied_live"]
        assert valid.json()["probe"] is None
    assert server.app.state.config.brain.providers["codex-subscription"].model == "new-native-model"


def test_subscription_cu_model_rejects_unverified_vision(server: WebServer) -> None:
    server.app.state.model_catalog = _SubscriptionCatalog()
    with TestClient(server.app) as client:
        invalid = client.put("/api/providers/codex-subscription/cu-model", json={"cu_model": "new-native-model", "persist": False})
        assert invalid.status_code == 422
        valid = client.put("/api/providers/codex-subscription/cu-model", json={"cu_model": "gpt-5.5", "persist": False})
        assert valid.status_code == 200
        assert valid.json()["effective_model"] == "gpt-5.5"


def test_subscription_cu_cannot_reset_to_unverified_main(server: WebServer) -> None:
    from jarvis.core.config import BrainProviderConfig

    server.app.state.model_catalog = _SubscriptionCatalog()
    server.app.state.config.brain.providers["codex-subscription"] = BrainProviderConfig(model="new-native-model", tool_model="gpt-5.5")
    with TestClient(server.app) as client:
        response = client.put("/api/providers/codex-subscription/cu-model", json={"cu_model": "", "persist": False})
        assert response.status_code == 422
    assert server.app.state.config.brain.providers["codex-subscription"].tool_model == "gpt-5.5"
