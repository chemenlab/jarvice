...
async def _run_screenshot_loop(
    ...
    *,
    ...
    def _final(stdout: str = "", stderr: str = "", exit_code: int = 0) -> HarnessResult:
        ...
        # Learn only from a verified final state. This runs after completion,
        # off the desktop/voice path, and writes historical wiki advice through
        # ToolExecutor. No generated executable skill is activated.
        try:
            from jarvis.skills.procedure_learning import queue_procedure

            queue_procedure(
                ctx.brain_manager, goal=task_prompt, steps=history,
                proof=stdout, exit_code=exit_code,
            )
        except Exception:
            log.warning("Procedure learning could not be queued", exc_info=True)
