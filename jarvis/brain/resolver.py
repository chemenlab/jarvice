...
def _tool_model_selection(config: JarvisConfig) -> tuple[str, str | None]:
    ...
    * The MODEL is a per-provider override — a user who picks a provider
    ...
    try:
        ...
        exclusive = _exclusive_selection(config)
...
def _exclusive_selection(config: JarvisConfig) -> tuple[str, str | None] | None:
    ...
    if spec is not None and spec.exclusive_main_brain:
        provider_cfg = config.brain.providers.get(primary)
        return primary, getattr(provider_cfg, "model", None)
    return None
