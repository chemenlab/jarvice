...
def _build_provider_catalog() -> dict[str, CatalogSpec]:
    ...
    # Main Codex brain uses the installed app-server's authenticated model/list,
    # independent of both the public API catalog and the old CLI worker list.
    cat["codex-subscription"] = CatalogSpec("brain", "model", (), live=True)
...
class ModelCatalog:
    ...
    async def _fetch_raw(self, provider: str) -> list[ModelInfo]:
        ...
        if provider == "codex-subscription":
            return await self._fetch_codex_subscription()
    ...
    @staticmethod
    async def _fetch_codex_subscription() -> list[ModelInfo]:
        """Ask Codex for its current roster without submitting a model turn."""
        ...
        try:
            async with asyncio.timeout(60):
                for _ in range(20):
                    params: dict = {"includeHidden": False, "limit": 100}
                    if cursor is not None:
                        params["cursor"] = cursor
                    result = await transport.request("model/list", params)
                    rows = result.get("data")
                    if not isinstance(rows, list):
                        raise RuntimeError("Invalid Codex model catalog response")
                    for row in rows:
                        ...
                        # Native user-image support alone does not prove that
                        # dynamic-tool image feedback works in Jarvis. Preserve
                        # only the integration capability verified end to end.
                        vision = (
                        ...
                        models[model_id] = ModelInfo(
                            ...
                            supported_parameters=(
                                ("tools",) if model_id in VERIFIED_MODELS else None
                            ),
                        )
                    ...
                    if not isinstance(cursor, str) or not cursor or cursor in seen_cursors:
                        raise RuntimeError("Invalid Codex model catalog pagination")
    ...
    def _static_fallback(self, provider: str) -> list[ModelInfo]:
        ...
        """
        if provider == "codex-subscription":
            # A configured default is not proof of this CLI account's roster.
            return []
