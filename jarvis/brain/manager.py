...
_PROVIDER_DISPLAY_NAMES: dict[str, str] = {
    ...
    "codex-subscription": "Codex CLI · ChatGPT subscription",
...
_REPLY_LANG_NAMES: dict[str, str] = {
    "de": "German", "en": "English", "es": "Spanish", "ru": "Russian"
}
...
_LANG_SWITCH_CONFIRM_SESSION: dict[str, str] = {
    ...
    "ru": "В этом разговоре отвечаю по-русски. Сохранить настройку не удалось.",
}
...
class BrainManager:
    ...
    def _exclusive_main_pick(self) -> tuple[str, str | None] | None:
        """Return a main selection whose reasoning must never be substituted."""
        from jarvis.ui.web.provider_spec import get_spec

        override = _TURN_OVERRIDE.get()
        provider = override.provider if override is not None else self._active_name
        spec = get_spec(provider)
        if spec is None or not spec.exclusive_main_brain:
            return None
        model = override.model if override is not None else self._fast_model(provider)
        return provider, model

    @property
    def _readback_composer(self) -> Any:
        if self._exclusive_main_pick() is not None:
            return None
        return self._api_readback_composer

    @_readback_composer.setter
    def _readback_composer(self, value: Any) -> None:
        self._api_readback_composer = value

    def _tool_model_provider(self) -> str:
        """Return the canonical Tool Model provider or its legacy fallback."""
        exclusive = self._exclusive_main_pick()
    ...
    def _reset_provider_caches(self) -> None:
        ...
        for instance in self._brain_cache.values():
            self._retire_brain(instance)
        self._brain_cache.clear()
        self._rate_tracker.clear()

    @staticmethod
    def _retire_brain(instance: Any) -> None:
        """Optional resource-owner hook; active turns finish on their old model."""
        retire = getattr(instance, "retire", None)
        if callable(retire):
            try:
                retire()
            except Exception:
                log.warning("Evicted brain cleanup could not be scheduled", exc_info=True)
    ...
    def reactivate_provider(self, provider: str) -> None:
        ...
        for k in keys_to_drop:
            self._retire_brain(self._brain_cache.pop(k, None))
    ...
    def apply_provider_model(self, provider: str, model: str) -> bool:
        ...
        for key in [
            ...
            self._retire_brain(self._brain_cache.pop(key, None))
    ...
    async def _generate_untagged(
        ...
        *,
        ...
        for idx, (prov_name, model) in enumerate(chain):
            ...
            # is not its truth.
            if (turn_override is None and self._exclusive_main_pick() is None
                    and prov_name in self._dead_providers):
                continue
            ...
            if (turn_override is None and self._exclusive_main_pick() is None
                    and (prov_name, model) in self._dead_provider_models):
                continue
        ...
        if used_provider is None:
            self._last_turn_all_failed = True
            if self._exclusive_main_pick() is not None:
                # Only the adapter's sanitized reason is safe for the UI.
                detail = getattr(last_exc, "public_message", None)
                if not detail:
                    detail = (
                        "Codex CLI недоступен. Проверьте вход ChatGPT и лимиты подписки "
                        "в настройках основного мозга."
                        if self._resolve_turn_lang() == "ru" else
                        "Codex CLI is unavailable. Check ChatGPT login and subscription "
                        "limits in Main brain settings."
                    )
                await self._publish_response_generated(trace_id=trace_uuid, text=detail)
                return detail
...
def _keyless_provider_is_rescued_by_oauth(provider_name: str) -> bool:
    ...
    if spec is not None and spec.exclusive_main_brain:
