    ...
    if tier == "router":
        ...
        if wiki_enabled:
            try:
                ...
                search = VaultSearch(
                    vault_path, db_path=resolve_wiki_db_path(config.memory.data_dir)
                )
...
def _build_flash_provider(jcfg: Any, ack_cfg: Any) -> Any:
    ...
    """
    # Subscription-main mode has one author for generated wording. Keep the
    # callers' deterministic pools, without resolving credentials or building
    # an independent API provider for acknowledgments and action readbacks.
    if getattr(getattr(jcfg, "brain", None), "primary", None) == "codex-subscription":
        return None
