/**
 * Zustand-backed interface translations for the desktop app.
 *
 * English, German, and Spanish core strings ship in the startup bundle.
 * Russian core strings and optional view chunks load on demand. Missing
 ...
 *   const lang = useUiLanguage();      // "en" | "de" | "es" | "ru"
 *   setUiLanguage("ru");               // updates subscribed views
 *
...
export const UI_LANGUAGES = ["en", "de", "es", "ru"] as const;
...
// Newer hydration requests or language choices invalidate pending responses.
let uiLanguageGeneration = 0;

export function isUiLanguage(v: unknown): v is UiLanguage {
  return typeof v === "string" && (UI_LANGUAGES as readonly string[]).includes(v);
}
...
// Keep the new locale off the startup path for existing installations.
let russianLocalePromise: Promise<void> | undefined;
export function loadUiLocale(language: UiLanguage): Promise<void> {
  if (language !== "ru") return Promise.resolve();
  return (russianLocalePromise ??= import("./locales/ru.json").then((module) => {
    RESOURCES.ru = module.default;
...
export async function hydrateUiLanguage(): Promise<void> {
  const generation = ++uiLanguageGeneration;
  try {
    const res = await fetch(UI_LANGUAGE_ENDPOINT);
    if (!res.ok) return;
    const body = (await res.json()) as { language?: unknown };
    if (isUiLanguage(body.language)) {
      await loadUiLocale(body.language);
      if (generation !== uiLanguageGeneration) return;
...
export const useI18nStore = create<I18nState>((set) => ({
  ...
  setUi: (lang, opts) => {
    uiLanguageGeneration += 1;
    void loadUiLocale(lang);
...
/**
 * Resolve "nav.skills" to a string in the active interface language.
 * Fallback order:
 *   1. Active language
 *   2. English (default)
 *   3. The key itself, so callers never receive undefined
 */
