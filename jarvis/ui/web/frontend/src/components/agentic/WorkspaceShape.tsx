  ...
  return (
    <div className="flex flex-col items-end gap-1.5">
      ...
      <div className="flex h-11 items-stretch overflow-hidden rounded-control border border-border bg-background transition-colors focus-within:border-primary/70 focus-within:ring-2 focus-within:ring-primary/15">
        <IconButton
          label={t("workspace_launcher.shape.fewer")}
          disabled={count <= 1}
          onClick={() => set(count - 1)}
        ...
        >
          <input
            ...
            spellCheck={false}
            value={draft}
            aria-label={t("workspace_launcher.shape.count_label")}
        ...
        <IconButton
          label={t("workspace_launcher.shape.more")}
          disabled={count >= max}
          onClick={() => set(count + 1)}
      ...
      >
        {ticks.map((n) => (
          <button
            ...
            aria-label={fill(t("workspace_launcher.shape.use_count"), { count: n })}
            onClick={() => onChange(n)}
...
function Readout({
  ...
  if (paneWidth === 0) {
    condition = t("workspace_launcher.shape.on_screen");
  } else if (perPane > 0 && perPane < WORKABLE_COLS) {
    // The one case worth spelling out, as advice: these panes come out narrow.
    condition = fill(t("workspace_launcher.shape.narrow_columns"), {
      columns: perPane,
      across: affordable,
      font_size: fontSize,
    });
  } else if (perPane > 0) {
    condition = fill(t("workspace_launcher.shape.fits_columns"), { columns: perPane });
  } else if (comfortable) {
    condition = fill(t("workspace_launcher.shape.fits_pixels"), {
      pixels: formatPx(paneWidth),
    });
  } else {
    condition = fill(t("workspace_launcher.shape.narrow_pixels"), {
      pixels: formatPx(paneWidth),
    });
  }
  ...
  return (
    ...
    >
      <span className="font-mono tabular-nums text-foreground">
        {fill(t("workspace_launcher.shape.across"), { count: columns })}
      </span>{" "}
      · {condition}
    </p>
  );
