...
vi.mock("@/i18n", async () => {
  const { default: english } = await import("@/i18n/locales/en.json");
  const shapeEnglish = Object.fromEntries(
    Object.entries(english.workspace_launcher.shape).map(([key, value]) => [
      `workspace_launcher.shape.${key}`,
      value,
    ]),
  );
  const translate = (key: string) => launcherEnglish[key] ?? shapeEnglish[key] ?? key;
  return {
    useT: () => translate,
