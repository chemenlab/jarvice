...
async def _run_tier_test(spec: ProviderSpec, cfg: Any, *, model: str | None = None) -> Any:
    ...
    """
    if spec.id == "codex-subscription":
        from .main_brain_routes import probe_subscription

        result = await probe_subscription()
        return _provider_test.ProviderTestResult(
            provider=spec.id,
            status="ok" if result.get("ready") else "not_configured",
            detail=(
                "ChatGPT login and Codex app-server verified. "
                "Subscription quota is checked when a turn runs."
                if result.get("ready") else result.get("error", "Codex is unavailable.")
            ),
            latency_ms=0.0,
        )
    family = _polish_family(spec)
...
def _current_brain_model(cfg: Any, provider: str) -> str:
    ...
    if provider == "codex-subscription" and not model:
        from jarvis.plugins.brain.codex_subscription import DEFAULT_MODEL
...
async def set_brain_model(
    ...
    if cat.tier == "brain":
        if provider_id == "codex-subscription":
            ...
            await _validate_subscription_model(request, value or DEFAULT_MODEL)
...
async def _validate_subscription_model(
    request: Request, model: str, *, vision: bool = False
) -> None:
    """Accept only the installed CLI roster and verified tool-image support."""
    result = await _get_model_catalog(request).list_models("codex-subscription")
    selected = next((entry for entry in result.models if entry.id == model), None)
    if selected is None:
        raise HTTPException(status_code=422, detail=(
            "Эта модель отсутствует в доступном списке Codex CLI. Обновите список моделей."
        ))
    if vision:
        from jarvis.plugins.brain.codex_subscription import VERIFIED_MODELS

        if model not in VERIFIED_MODELS or "image" not in (selected.input_modalities or ()):
            raise HTTPException(status_code=422, detail=(
                "Для инструментов с изображениями выберите проверенную модель: "
...
async def set_cu_model(provider_id: str, body: CuModelBody, request: Request) -> CuModelResponse:
    ...
    if provider_id == "codex-subscription":
        effective = value or _current_brain_model(_resolve_cfg(request), provider_id)
        await _validate_subscription_model(request, effective, vision=True)
...
async def brain_switch(body: SwitchBody, request: Request) -> dict[str, Any]:
    ...
    """
    if body.provider == "codex-subscription":
        from .main_brain_routes import MainBrainSelection, select_main_brain

        return await select_main_brain(MainBrainSelection(provider=body.provider), request)
