...
# one view rather than a window flag (2026-08-24 report: text could not be copied).
#
...
def _voice_brain_for_config(config: Any, brain: Any) -> Any:
    """Use the shared brain; subscription mode never creates a voice fallback."""
    ...
    if callable(brain) and not hasattr(brain, "respond"):
        return brain
    if shared_subscription_main:
    ...
    logger.info("Shared brain is not directly callable — building a dedicated voice brain.")
    return build_default_brain(tier="router")
...
class DesktopApp:
    ...
    async def _start_speech_and_orb(
        ...
        try:
            ...
            voice_brain = _voice_brain_for_config(self.cfg, brain)
