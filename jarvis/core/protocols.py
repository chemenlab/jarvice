...
class BrainRequest:
    ...
    # Opaque execution scope owned by one ToolUseLoop.run, shared by its
    # model rounds. Stateful transports use it to correlate suspended tool
    # calls with the next request's results. Never sent as model content.
    # None preserves standalone completions and existing provider callers.
