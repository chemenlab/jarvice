"""Turn-language detection and resolution (de / en / es / ru).
...
# Vocabulary, not Cyrillic alone: other languages and proper names also use
# Cyrillic. An English sentence mentioning a Russian name must stay English.
_RU_TOKENS: frozenset[str] = frozenset({
    "я", "ты", "вы", "мы", "мне", "меня", "тебя", "тебе", "нам", "вам",
    "мой", "моя", "мои", "твой", "наш", "это", "этот", "эта", "эти",
    "что", "как", "когда", "где", "почему", "какой", "какая", "какие",
    "кто", "сколько", "пожалуйста", "спасибо", "привет", "да", "нет",
    "хочу", "нужно", "можно", "можешь", "будет", "сегодня", "завтра",
    "вчера", "сейчас", "потом", "погода", "скажи", "покажи", "открой",
    "найди", "прочитай", "напиши", "проверь", "напомни", "сделай",
})
...
def normalize_language_tag(tag: object) -> str:
    """Collapse an STT/TTS tag to a supported language code or ``unknown``."""
    if not tag:
        return "unknown"
...
def detect_text_language(text: str) -> str:
    """Classify *text* by supported vocabulary, or ``unknown`` when unclear.
...
def _detect_strong_supported_output(text: str, tokens: list[str]) -> str:
    """Return a supported code when multiple independent prose signals agree."""
    if len(tokens) < 4:
        return "unknown"
    scores = {
...
def validate_output_language(
    ...
    *,
    ...
    """Validate reply *text* against an already-resolved language code.
