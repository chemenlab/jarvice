...
def set_subscription_main_brain(
    *, model: str = "gpt-5.5", path: Path = DEFAULT_CONFIG_FILE
) -> None:
    """Atomically select Codex reasoning with Gemini speech-only input/output.

    The native app applies this profile on restart. Preserve the prior provider
    and speech selection in TOML so returning to the previous setup is possible.
    No credential values are read, copied, or changed here.
    """
    path = _ensure_writable_config_path(path)
    with _WRITE_LOCK:
        raw = path.read_text(encoding="utf-8")
        had_bom = raw.startswith(_BOM)
        doc = tomlkit.parse(raw.removeprefix(_BOM))

        def table(parent, key, *, super_table=False):
            if key not in parent:
                parent[key] = tomlkit.table(super_table)
            return parent[key]

        brain = table(doc, "brain")
        voice = table(doc, "voice")
        stt = table(doc, "stt")
        tts = table(doc, "tts")
        if "subscription_main_restore" not in doc:
            restore = table(doc, "subscription_main_restore")
            for key, value in {
                "brain_provider": brain.get("primary", ""),
                "voice_mode": voice.get("mode", "pipeline"),
                "voice_profile": voice.get("profile", ""),
                "stt_provider": stt.get("provider", ""),
                "tts_provider": tts.get("provider", ""),
            }.items():
                restore[key] = value
        brain["primary"] = "codex-subscription"
        brain["deep_brain"] = "codex-subscription"
        table(brain, "router")["provider"] = "codex-subscription"
        # The shared main brain inherits the provider's one persisted picker.
        # A second explicit router pin would undo later model changes on boot.
        table(brain, "router")["model"] = ""
        table(brain, "tool_model")["provider"] = "codex-subscription"
        providers = table(brain, "providers", super_table=True)
        table(providers, "codex-subscription")["model"] = model
        voice["mode"] = "pipeline"
        voice["profile"] = ""
        stt["provider"] = "gemini-api"
        tts["provider"] = "gemini-flash-tts"
        _atomic_write(path, (_BOM if had_bom else "") + tomlkit.dumps(doc))
    _sync_brain_primary_drift_soll("codex-subscription")  # i18n-allow: existing persistence identifier
    _sync_brain_provider_model_drift_soll("codex-subscription", model=model, deep_model=None)  # i18n-allow: existing persistence identifier
    _sync_stt_provider_drift_soll("gemini-api")  # i18n-allow: existing persistence identifier
    _sync_tts_provider_drift_soll({"provider": "gemini-flash-tts"})  # i18n-allow: existing persistence identifier
...
def set_ui_language(name: str, *, path: Path = DEFAULT_CONFIG_FILE) -> None:
    ...
    ``name`` is one of ``en`` | ``de`` | ``es`` | ``ru`` (validated by the caller). This
...
def set_wake_language(language: str, *, path: Path = DEFAULT_CONFIG_FILE) -> None:
    ...
    One of ``auto`` | ``de`` | ``en`` | ``es`` | ``ru`` (validated by the caller). This is
...
def _patch_wake_word_toml(path: Path, values: dict[str, object]) -> None:
    ...
    with _WRITE_LOCK:
        ...
        # Repair an old literal dotted table without losing user-tuned fields.
        # Valid nested values win; the requested update wins over both.
        legacy = doc.get("trigger.wake_word")
        if isinstance(legacy, dict):
            for key, value in legacy.items():
                if key not in wake_word:
                    wake_word[key] = value
        for key, value in values.items():
            wake_word[key] = value
