...
async def _close_attempt(brain: Any, stream: Any = None) -> None:
    """Release the per-attempt stream and optional provider-owned resources."""
    for resource, method in ((stream, "aclose"), (brain, "close")):
        ...
        if callable(close):
            try:
                ...
                log.warning("Wiki provider cleanup failed (%s)", type(exc).__name__)
    ...
    *,
    ...
    for index, (provider, model) in enumerate(ordered):
        ...
        try:
            ...
            await _close_attempt(brain, stream)
            detail = _exception_summary(exc)
            log.error(
