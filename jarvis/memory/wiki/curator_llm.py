...
class WikiCuratorLLM:
    ...
    async def propose_updates(
        ...
        * every update in the response failed validation.
        ...
        candidate_slugs = await asyncio.to_thread(
            self._collect_candidate_slugs, vault, vault_summary,
        )
        ...
        def _page_context() -> list[dict[str, Any]]:
            pages = []
            for page_type in ("entity", "concept", "project", "session"):
                try:
                    pages.extend(vault.pages_by_type(page_type) or [])
                except Exception as exc:  # noqa: BLE001 - index failure must not overwrite memory
                    logger.warning("Cannot load existing wiki pages for %s: %s", page_type, exc)
                    raise
            return select_existing_page_context(
                source_content, pages, repo,
                max_chars=min(48_000, max(
                    0, self._cfg.max_input_tokens * 2 - len(system_prompt) - len(source_content),
                )),
            )

        try:
            existing_pages = await asyncio.to_thread(_page_context)
        except Exception:  # noqa: BLE001 - logged by the reader; no ungrounded replacements
            return []
        user_prompt = build_user_prompt(
            source_label, source_content, top_slugs, existing_pages=existing_pages,
        )
        ...
        def _validate_response(agg: Any) -> str | None:
            ...
            try:
                ...
                rejection_reasons.append(reason)
                return reason
            omitted_paths = {page["path"] for page in existing_pages if not page["complete"]}
            if any(
                update.target_path.as_posix() in omitted_paths
                or (update.rename_from and update.rename_from.as_posix() in omitted_paths)
                for update in updates
            ):
                reason = "cannot replace an existing page without its complete content"
                rejection_reasons.append(reason)
                return reason
