...
_NORMALISE_RE = re.compile(r"[\W_]+", re.UNICODE)
_MATCH_NORMALISE_RE = _NORMALISE_RE
...
def normalize_phrase(phrase: str) -> list[str]:
    ...
    Keeps Unicode letters and digits. Empty/whitespace input returns ``[]``.
    """
...
def _vosk_models_root() -> Path:
    ...
    offline afterwards.
    """
    base = os.environ.get("JARVIS__MEMORY__DATA_DIR") or os.environ.get("JARVIS_DATA_DIR") or "data"
