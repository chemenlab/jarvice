...
class SpeechPipeline:
    ...
    def __init__(
        ...
        shared_subscription_main = (
            getattr(getattr(config, "brain", None), "primary", None) == "codex-subscription"
            or getattr(brain_callback, "active_provider", None) == "codex-subscription"
        )
        if shared_subscription_main and not callable(brain_callback):
            raise ValueError("Codex subscription voice requires the shared brain callback")
        ...
        if config is not None:
            ...
            if subscription_voice_selected(config) and not shared_subscription_main:
                self._brain = CodexSubscriptionVoiceBrain(self._brain, config)
        # Flash-Brain reference (None when feature disabled).
        self._ack_brain: Any = None if shared_subscription_main else ack_brain
    ...
    async def _on_announcement(self, event: AnnouncementRequested) -> None:
        ...
        if (
            ...
            and not self._uses_shared_subscription_brain()
    ...
    async def _instant_ack_body(
        ...
        try:
            ...
            if not self._instant_ack_still_wanted(seq):
                return
            # The subscription main brain owns all generated wording. Read
            # this per turn so a live switch also disables an existing API
            # composer, while the deterministic status pools remain available.
            composer = (
                None if self._uses_shared_subscription_brain()
                else getattr(self._brain, "_readback_composer", None)
            )
            if plan.contextual:
    ...
    async def _spawn_flash_brain_ack(self, utterance: str, language: str) -> None:
        ...
        """
        if self._ack_brain is None or self._uses_shared_subscription_brain():
            return
            ...
            try:
                async for sentence in run_stream(utterance, language=language):
                    ...
                    if self._turn_state is not TurnTakingState.PROCESSING:
                        ...
                        return
                    if self._uses_shared_subscription_brain():
                        return
                    tracker = getattr(self, "_latency_tracker", None)
    ...
    def apply_voice_profile(self, profile: str) -> bool:
        ...
        if voice is not None:
            ...
            if normalized:
                voice.mode = "pipeline"
        if normalized and not self._uses_shared_subscription_brain():
    ...
    def _uses_shared_subscription_brain(self) -> bool:
        """Keep voice on the selected main brain across live provider changes."""
        config = getattr(self, "_config", None)
        brain = getattr(self, "_base_brain", getattr(self, "_brain", None))
        return (
            getattr(getattr(config, "brain", None), "primary", None) == "codex-subscription"
            or getattr(brain, "active_provider", None) == "codex-subscription"
        )
    ...
    _BCP47: dict[str, str] = {
        "de": "de-DE", "en": "en-US", "es": "es-ES", "ru": "ru-RU",
    }

    @classmethod
    def _bcp47(cls, lang: object) -> str | None:
        """Map a supported turn-language code to a TTS BCP-47 locale, else None.
